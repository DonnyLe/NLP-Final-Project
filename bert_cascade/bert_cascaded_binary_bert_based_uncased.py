...
def random_search_kfold(
    ...
    print(
        ...
        f"{n_trials} trials, {n_splits}-fold CV"
    )
...
def run_cascade_experiment(
    ...
    ax.set_title(f"Cascaded BERT Confusion Matrix - {transcript_col}")
    ...
    with open(f"bert_cascade_classification_report_{transcript_col}.txt", "w") as f:
        f.write(f"Cascaded 3-class report for {transcript_col}\n\n")
        f.write(report)
        f.write("\n")
...
if __name__ == "__main__":
    ...
    for transcript_col in TRANSCRIPT_COLS_TO_RUN:
        ...
        ax.set_title(f"Confusion Matrix - HC vs Non-HC - {transcript_col}")
        ...
        with open(f"bert_classification_report_HC_vs_NonHC_{transcript_col}.txt", "w") as f:
            f.write(f"Binary HC vs Non-HC report for {transcript_col}\n\n")
            f.write(report_A)
            f.write("\n")
        ...
        ax.set_title(f"Confusion Matrix - MCI vs Dementia - {transcript_col}")
        ...
        with open(f"bert_classification_report_MCI_vs_Dem_{transcript_col}.txt", "w") as f:
            f.write(f"Binary MCI vs Dementia report for {transcript_col}\n\n")
            f.write(report_B)
            f.write("\n")
    ...
    print("\nAll cascaded experiments finished for PFT / CTD / SFT.")
